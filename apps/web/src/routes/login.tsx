...
const oauthButtons: OAuthButtonConfig[] = [
	...
	},
	{
		...
		className: "btn-outline hover:bg-[#5865F2]/20 hover:border-[#5865F2]",
	},
	{
		provider: "github",
		label: "GitHubでログイン",
	...
	const handleOAuthLogin = async (provider: OAuthProvider) => {
		...
		try {
			await authClient.signIn.social({
				provider,
				callbackURL: validatedReturnTo || "/auth/callback",
			});
		} catch (err) {
		...
		<div className="flex min-h-screen items-center justify-center bg-base-200">
			<div className="card w-full max-w-md border border-base-300 bg-base-100">
				<div className="card-body p-8">
					<h1 className="card-title justify-center text-2xl">ログイン</h1>
					...
					<div className="mt-8 space-y-5">
						{oauthButtons.map(({ provider, label, icon, className }) => (
							<Button
								key={provider}
								onClick={() => handleOAuthLogin(provider)}
								disabled={loadingProvider !== null}
								className={`btn-lg w-full gap-3 ${className}`}
							>
