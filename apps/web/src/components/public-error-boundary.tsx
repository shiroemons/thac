...
/**
 * 公開ルート用エラーバウンダリコンポーネント
 * 認証不要なページでerrorComponentとして使用
 */
export function PublicErrorBoundary({ error }: ErrorComponentProps) {
	const router = useRouter();
	const isTimeout = isTimeoutError(error);
	const isSSR = isSSRError(error);

	// エラーログ出力
	console.error(`[${isSSR ? "SSR" : "Client"}] Public Route Error:`, {
