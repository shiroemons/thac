...
type TrackRow = {
	...
	releaseReleaseDate: string | null;
	releaseReleaseYear: number | null;
	releaseType: string | null;
	releaseEventId: string | null;
	discNumber: number | null;
	discName: string | null;
	trackEventId: string | null;
	trackEventName: string | null;
};

/**
 * Transform raw track rows into search documents by batch-fetching related data
 * Shared logic used by both fetchTracksForIndexing and fetchTracksForIndexingByIds
 */
async function transformTracksData(
	tracksData: TrackRow[],
): Promise<TrackSearchDocument[]> {
...
export async function* fetchTracksForIndexing(
	...
	while (hasMore) {
		...
		const documents = await transformTracksData(tracksData);
		yield documents;
...
/**
 * Fetch specific tracks by IDs for indexing, yielding batches
 * Used for bulk sync after imports (avoids N+1 pattern of fetchTrackForIndexing)
 * @param trackIds - Array of track IDs to index
 * @param batchSize - Number of tracks per batch (default: 500)
 */
export async function* fetchTracksForIndexingByIds(
	trackIds: string[],
	batchSize = 500,
): AsyncGenerator<TrackSearchDocument[]> {
	for (let i = 0; i < trackIds.length; i += batchSize) {
		...
		// Fetch basic track data with release, disc, and events
		const tracksData = await db
			.select({
				id: tracks.id,
				name: tracks.name,
			...
			.leftJoin(discs, eq(tracks.discId, discs.id))
			.leftJoin(events, eq(tracks.eventId, events.id))
			.where(inArray(tracks.id, batchIds));
